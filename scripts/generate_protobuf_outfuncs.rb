...
class Generator
      ...
      @struct_defs[group].each do |node_type, struct_def|
        ...
        struct_def['fields'].each do |field_def|
          ...
          if type == :skip
            ...
            protobuf_field_count += 1
          elsif ['uint', 'uint16', 'uint32', 'Index', 'bits32', 'Oid', 'AclMode'].include?(type)
            ...
            @outmethods[node_type] += format("  WRITE_SPECIFIC_NODE_FIELD(%s, %s, %s);\n", type.gsub('*', ''), underscore(name), name)
            @protobuf_messages[node_type] += format("  %s %s = %d [json_name=\"%s\"];\n", type.gsub('*', ''), underscore(name), protobuf_field_count, name)
            protobuf_field_count += 1
          elsif @nodetypes.include?(type[0..-2])
            @outmethods[node_type] += format("  WRITE_SPECIFIC_NODE_PTR_FIELD(%s, %s, %s);\n", type.gsub('*', ''), underscore(name), name)
            @protobuf_messages[node_type] += format("  %s %s = %d [json_name=\"%s\"];\n", type.gsub('*', ''), underscore(name), protobuf_field_count, name)
            protobuf_field_count += 1
          elsif type.end_with?('*')
            puts format('ERR: %s %s', name, type)
          else # Enum
            @outmethods[node_type] += format("  WRITE_ENUM_FIELD(%s, %s, %s);\n", type, underscore(name), name)
            @protobuf_messages[node_type] += format("  %s %s = %d [json_name=\"%s\"];\n", type, underscore(name), protobuf_field_count, name)
            protobuf_field_count += 1
          end
        end
      end
  ...
  def generate!
    ...
    @nodetypes.each do |type|
      ...
      defs += format("static void _out%s(pg_query::%s * out_node, const %s *node);\n", type, type, type)

      impls += "static void\n"
      impls += format("_out%s(pg_query::%s * out_node, const %s *node)\n", type, type, type)
      impls += "{\n"
      impls += outmethod
      impls += "}\n"
      impls += "\n"
      ...
      conds += format("  OUT_NODE(%s, %s, %s);\n", type, type, underscore(type))
      conds += "  break;\n"
...
"
    ...
    File.write('./src/pg_query_protobuf_defs.cc', defs + impls)
    File.write('./src/pg_query_protobuf_conds.cc', conds)
